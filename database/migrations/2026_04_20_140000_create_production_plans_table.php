...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('production_plans', function (Blueprint $table): void {
            $table->id();
            $table->string('plan_number')->unique()->comment('Gyártási terv egyedi azonosítója');
            $table->dateTime('target_at')->index()->comment('Gyártási terv célideje');
            $table->string('status')->default('draft')->index()->comment('Gyártási terv állapota: draft, calculated, ready, archived');
            ...
            $table->boolean('is_locked')->default(false)->index()->comment('Gyártási terv zárolt-e');
            $table->text('notes')->nullable()->comment('Belso megjegyzes');
            $table->foreignId('created_by')->nullable()->constrained('users')->nullOnDelete()->comment('Gyártási tervet létrehozó felhasználó');
            $table->timestamps();
        });
    }
