...
<template>
    <Head :title="$t('orders.success')" />

    <section class="mx-auto max-w-3xl space-y-6">
        <header class="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 sm:p-8">
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">Hajnalhej Bakery</p>
            <h1 class="mt-2 font-heading text-4xl text-bakery-dark">
                {{ $t("orders.thank_you_order") }}
            </h1>
            <p class="mt-2 text-sm text-bakery-dark/75">
                {{ $t("orders.order_registered") }}
            </p>
        </header>
        ...
        <article class="rounded-2xl border border-bakery-brown/15 bg-[#fff9f1] p-5">
            <dl class="grid gap-3 text-sm sm:grid-cols-2">
                <div>
                    <dt class="text-bakery-dark/60">{{ $t("orders.order_id") }}</dt>
                ...
                <div>
                    <dt class="text-bakery-dark/60">{{ $t("common.pickup_date") }}</dt>
                    <dd class="font-semibold text-bakery-dark">
                        {{ order.pickup_date || "-" }}
                    </dd>
                </div>
