...
<template>
    <Head :title="$t('checkout.page.meta_title')" />

    <section class="mx-auto max-w-6xl space-y-6">
        <header class="rounded-3xl border border-bakery-brown/15 bg-[#fff7eb] p-6 sm:p-8">
            <h1 class="font-heading text-4xl text-bakery-dark">{{ $t('checkout.page.title') }}</h1>
            <p class="mt-2 text-sm text-bakery-dark/75">{{ $t('checkout.page.subtitle') }}</p>
        </header>
            ...
            <form class="space-y-4 rounded-2xl border border-bakery-brown/15 bg-white/80 p-5" @submit.prevent="submit">
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="space-y-2 sm:col-span-2">
                        <label for="customer_name" class="text-sm font-medium text-bakery-dark">{{ $t('checkout.page.customer_name') }}</label>
                    ...
                    <div class="space-y-2">
                        <label for="customer_email" class="text-sm font-medium text-bakery-dark">{{ $t('checkout.page.customer_email') }}</label>
                    ...
                    <div class="space-y-2">
                        <label for="customer_phone" class="text-sm font-medium text-bakery-dark">{{ $t('checkout.page.customer_phone') }}</label>
                    ...
                    <div class="space-y-2">
                        <label for="pickup_date" class="text-sm font-medium text-bakery-dark">{{ $t('checkout.page.pickup_date') }}</label>
                    ...
                    <div class="space-y-2">
                        <label for="pickup_time_slot" class="text-sm font-medium text-bakery-dark">{{ $t('checkout.page.pickup_time_slot') }}</label>
                        <InputText id="pickup_time_slot" v-model="form.pickup_time_slot" :placeholder="$t('checkout.page.pickup_time_placeholder')" class="w-full" :invalid="Boolean(form.errors.pickup_time_slot)" />
                        <p v-if="form.errors.pickup_time_slot" class="text-xs text-red-700">{{ form.errors.pickup_time_slot }}</p>
                    </div>

                    <div class="space-y-2 sm:col-span-2">
                        <label for="notes" class="text-sm font-medium text-bakery-dark">{{ $t('checkout.page.notes') }}</label>
                ...
                >
                    <Checkbox
                        ...
                        class="mt-0.5"
                    />
                    <span>{{ $t('checkout.page.accept_privacy') }}</span>
                </label>
                ...
                >
                    <Checkbox
                        ...
                        class="mt-0.5"
                    />
                    <span>{{ $t('checkout.page.accept_terms') }}</span>
                </label>
                <p v-if="form.errors.accept_terms" class="text-xs text-red-700">{{ form.errors.accept_terms }}</p>

                <Button type="submit" :label="$t('checkout.page.submit_order')" class="w-full" :loading="form.processing" :disabled="form.processing" />
            </form>

            <aside class="h-fit rounded-2xl border border-bakery-brown/15 bg-[#fff9f1] p-5 shadow-sm">
                <h2 class="font-heading text-2xl text-bakery-dark">{{ $t('checkout.page.order_summary') }}</h2>
                ...
                <div class="mt-5 border-t border-bakery-brown/10 pt-4 text-sm">
                    <div class="flex justify-between font-semibold text-bakery-dark">
                        <span>{{ $t('checkout.page.total_label') }}</span>
