...
const registryStateOptions = [
    ...
    { label: trans("admin_permissions.registry_states.missing_in_db"), value: "missing_in_db" },
    { label: trans("admin_permissions.registry_states.orphan_db_only"), value: "orphan_db_only" },
];
...
const sortFieldOptions = [
    ...
    { label: trans("admin_permissions.sort_fields.registry_state"), value: "registry_state" },
];
...
const perPageOptions = [
    { label: trans("admin_permissions.filters.per_page_option", { count: 20 }), value: 20 },
    { label: trans("admin_permissions.filters.per_page_option", { count: 50 }), value: 50 },
    { label: trans("admin_permissions.filters.per_page_option", { count: 100 }), value: 100 },
];
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-bakery-brown/15 bg-white/80 p-4 sm:p-5">
            <AdminTableToolbar :filters-grid-class="'grid gap-3 md:grid-cols-2 xl:grid-cols-4'">
                <template #filters>
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.search') }}</label>
                        <InputText v-model="filterState.search" class="w-full" :placeholder="$t('admin_permissions.filters.search_placeholder')" @keyup.enter="submitFilters" />
                    </div>

                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.module') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.usage') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.registry_state') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.sort_field') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.sort_direction') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.per_page') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-medium uppercase tracking-[0.14em] text-bakery-brown/80">{{ $t('admin_permissions.filters.dangerous_only') }}</label>
                        <div class="flex h-10 items-center gap-2 rounded-lg border border-bakery-brown/15 px-3">
                            ...
                            <label for="dangerous-only" class="text-sm text-bakery-dark">{{ $t('admin_permissions.filters.dangerous_only') }}</label>
                        </div>
                    </div>
                </template>

                <template #actions>
                    <Button icon="pi pi-search" :label="$t('admin_permissions.actions.filter')" @click="submitFilters" />
                    <Button
                        v-if="can.sync"
                        icon="pi pi-sync"
                    ...
                    <template #empty>
                        <div class="rounded-xl border border-dashed border-bakery-brown/25 bg-[#fcf7ef] p-6 text-center text-sm text-bakery-dark/70">
                            ...
                            <div class="mt-3 flex flex-wrap items-center justify-center gap-2">
                                ...
                                <Button v-if="can.sync" :label="$t('admin_permissions.actions.registry_sync')" size="small" @click="runSync" />
                            </div>
                        </div>
                    </template>
