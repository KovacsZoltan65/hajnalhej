    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-4 rounded-2xl border border-bakery-brown/15 bg-white/80 p-4 sm:grid-cols-2 sm:p-5">
            <div>
                ...
                <div class="mt-2">
                    <AuditEventBadge :event-key="log.event" :label="eventLabels[log.event] ?? log.event" />
                </div>
            </div>
            <div>
                <p class="text-xs uppercase tracking-[0.16em] text-bakery-brown/70">
            ...
            <div>
                ...
                <p class="mt-2 text-sm font-semibold uppercase tracking-widest text-bakery-dark">
                    {{ log.log_name }}
                </p>
            </div>
            <div>
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            <article class="rounded-2xl border border-bakery-brown/15 bg-white/80 p-4">
                ...
                <pre class="mt-3 overflow-x-auto rounded-lg bg-[#fff9f1] p-3 text-xs text-bakery-dark">{{
                    formatJson(log.properties.before)
                }}</pre>
            </article>
            ...
            <article class="rounded-2xl border border-bakery-brown/15 bg-white/80 p-4">
                ...
                <pre class="mt-3 overflow-x-auto rounded-lg bg-[#fff9f1] p-3 text-xs text-bakery-dark">{{
                    formatJson(log.properties.after)
                }}</pre>
            </article>
            ...
            <article class="rounded-2xl border border-bakery-brown/15 bg-white/80 p-4">
                ...
                <pre class="mt-3 overflow-x-auto rounded-lg bg-[#fff9f1] p-3 text-xs text-bakery-dark">{{
                    formatJson(log.properties.context)
                }}</pre>
            </article>
