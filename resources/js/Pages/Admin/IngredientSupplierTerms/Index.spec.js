...
const { translate } = vi.hoisted(() => {
    const translations = {
        ...
        "admin_supplier_terms.confirm_delete_message": "Biztosan törlöd ezt a feltételt: :ingredient / :supplier?",
        "common.all": "Mind",
        "common.active": "Aktív",
...
const stubs = {
    ...
    DataTable: {
        props: ["value"],
        template:
            '<div><div v-for="row in value" :key="row.id">{{ row.ingredient_name }} {{ row.supplier_name }}</div><slot /></div>',
    },
    ...
    PreferredBadge: {
        props: ["preferred", "active"],
        template: '<span>{{ preferred && active ? "Preferált" : "Normál" }}</span>',
    },
    SectionTitle: {
...
describe("IngredientSupplierTerms Index", () => {
    ...
    it("renders the supplier terms list and opens create modal", async () => {
        ...
        await wrapper
            .findAll("button")
            .find((button) => button.text() === "Új feltétel")
            .trigger("click");
