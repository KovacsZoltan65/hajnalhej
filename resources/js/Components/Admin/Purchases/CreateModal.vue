<script setup>
import Button from 'primevue/button';
import Dialog from 'primevue/dialog';
import PurchaseForm from './PurchaseForm.vue';

defineProps({
    visible: {
        type: Boolean,
        required: true,
    },
    form: {
        type: Object,
        required: true,
    },
    suppliers: {
        type: Array,
        required: true,
    },
    ingredientOptions: {
        type: Array,
        required: true,
    },
});

const emit = defineEmits(['update:visible', 'submit']);

const close = () => emit('update:visible', false);
</script>

<template>
    <Dialog
        :visible="visible"
        modal
        header="Új beszerzés"
        :style="{ width: '64rem', maxWidth: '97vw' }"
        :content-style="{ maxHeight: '70vh', overflowY: 'auto' }"
        @update:visible="(value) => emit('update:visible', value)"
    >
        <form id="purchase-create-form" class="space-y-4" @submit.prevent="emit('submit')">
            <PurchaseForm :form="form" :suppliers="suppliers" :ingredient-options="ingredientOptions" />
        </form>

        <template #footer>
            <div class="flex justify-end gap-2">
                <Button type="button" severity="secondary" label="Mégse" @click="close" />
                <Button type="submit" form="purchase-create-form" label="Mentés" :loading="form.processing" />
            </div>
        </template>
    </Dialog>
</template>
