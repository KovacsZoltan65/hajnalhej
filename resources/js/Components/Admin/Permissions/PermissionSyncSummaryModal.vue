...
<template>
    <Dialog
        ...
        modal
        header="Permission sync summary"
        ...
        <div v-if="props.summary" class="space-y-4">
            <div class="grid gap-3 sm:grid-cols-3">
                <div class="rounded-lg bg-emerald-50 p-3">
                    <p class="text-xs uppercase tracking-[0.12em] text-emerald-700">Created</p>
                    <p class="text-xl font-semibold text-emerald-800">{{ props.summary.created_count ?? 0 }}</p>
                </div>
                <div class="rounded-lg bg-slate-50 p-3">
                    <p class="text-xs uppercase tracking-[0.12em] text-slate-700">Existing</p>
                    <p class="text-xl font-semibold text-slate-800">{{ props.summary.existing_count ?? 0 }}</p>
                </div>
                <div class="rounded-lg bg-amber-50 p-3">
                    <p class="text-xs uppercase tracking-[0.12em] text-amber-700">Orphans</p>
            ...
            <div class="space-y-2 rounded-lg border border-bakery-brown/15 bg-[#fff9f1] p-3 text-sm text-bakery-dark">
                <p><span class="font-medium">Created permissions:</span> {{ (props.summary.created_permissions ?? []).join(', ') || 'Nincs' }}</p>
                <p><span class="font-medium">Orphan permissions:</span> {{ (props.summary.orphan_permissions ?? []).join(', ') || 'Nincs' }}</p>
            </div>
        </div>
