...
<template>
    <Dialog
        :visible="visible"
        modal
        :header="$t('admin_branches.actions.edit')"
        :style="{ width: '44rem', maxWidth: '95vw' }"
        :content-style="{ maxHeight: '70vh', overflowY: 'auto' }"
        @update:visible="(value) => emit('update:visible', value)"
    >
        <form id="branch-edit-form" class="space-y-4" @submit.prevent="emit('submit')">
        ...
        <template #footer>
            <div class="flex justify-end gap-2">
                ...
                <Button type="submit" form="branch-edit-form" :label="$t('common.save')" :loading="form.processing" />
            </div>
        </template>
    </Dialog>
</template>
