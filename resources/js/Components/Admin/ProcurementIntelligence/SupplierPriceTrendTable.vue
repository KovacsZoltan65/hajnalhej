    ...
    <section class="ui-card p-4 sm:p-5">
        <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs text-bakery-dark/60">
                {{
                    trans("admin_procurement_intelligence.counts.rows", {
                        count: rows.length,
                    })
                }}
            </span>
                ...
                >
                    <tr>
                        ...
                        <th class="px-2 py-2 text-right">
                            {{
                                trans(
                                    "admin_procurement_intelligence.columns.previous_price"
                                )
                            }}
                        </th>
                        <th class="px-2 py-2 text-right">
                            {{ trans("admin_procurement_intelligence.columns.change") }}
                        </th>
                        <th class="px-2 py-2 text-right">
                            {{
                                trans(
                                    "admin_procurement_intelligence.columns.change_percent"
                                )
                            }}
                        </th>
                        <th class="px-2 py-2">
                            {{ trans("admin_procurement_intelligence.columns.cheapest") }}
                        </th>
                        <th class="px-2 py-2">
                            {{
                                trans(
                                    "admin_procurement_intelligence.columns.most_expensive"
                                )
                            }}
                        </th>
                        <th class="px-2 py-2">
                            {{ trans("admin_procurement_intelligence.columns.trend") }}
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr
                        v-for="row in rows"
                        :key="`${row.ingredient_id}-${row.supplier_id ?? 'none'}`"
                        class="border-b border-bakery-brown/10"
                    >
                        <td class="px-2 py-3 font-medium text-bakery-dark">
                            {{ row.ingredient_name }}
                        </td>
                        <td class="px-2 py-3 text-bakery-dark/80">
                            {{ row.supplier_name }}
                        </td>
                        <td class="px-2 py-3 text-right text-bakery-dark">
                            {{ formatCurrency(row.last_unit_cost) }}
                        </td>
                        <td class="px-2 py-3 text-right text-bakery-dark">
                            {{ formatCurrency(row.previous_unit_cost) }}
                        </td>
                        <td class="px-2 py-3 text-right text-bakery-dark">
                            {{ formatCurrency(row.change_amount) }}
                        </td>
                        <td
                            class="px-2 py-3 text-right font-semibold"
                            :class="
                        ...
                        <td class="px-2 py-3 text-bakery-dark/80">
                            {{ row.cheapest_supplier?.supplier_name ?? "-" }}
                        </td>
                        <td class="px-2 py-3 text-bakery-dark/80">
                            {{ row.most_expensive_supplier?.supplier_name ?? "-" }}
                        </td>
