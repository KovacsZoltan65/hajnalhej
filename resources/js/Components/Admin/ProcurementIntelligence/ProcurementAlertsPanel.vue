    ...
    <section class="ui-card p-4 sm:p-5">
        ...
        <div v-if="alerts.length" class="mt-4 grid gap-3">
            <article
                ...
                :key="`${alert.type}-${alert.ingredient_id}-${index}`"
                class="rounded-lg border border-bakery-brown/15 p-4"
            >
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.12em] text-bakery-dark/55">
