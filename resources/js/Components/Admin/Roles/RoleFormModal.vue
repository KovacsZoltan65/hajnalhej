    ...
    >
        <form id="role-form" class="space-y-4" @submit.prevent="emit('submit')">
            <div class="space-y-2">
                ...
                />
                <p class="text-xs text-bakery-dark/65">Kisbetu, szam, pont, kotojel vagy alahuzas hasznalhato.</p>
                <p v-if="props.form.errors.name" class="text-xs text-red-700">{{ props.form.errors.name }}</p>
            </div>
        </form>
