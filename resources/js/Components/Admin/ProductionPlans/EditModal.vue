...
<template>
    ...
    >
        <form id="production-plan-edit-form" class="space-y-4" @submit.prevent="emit('submit')">
            ...
            <div
                v-if="summary"
                class="grid gap-3 rounded-xl border border-bakery-brown/15 bg-[#fcf8f1] p-4 sm:grid-cols-2 xl:grid-cols-4"
            >
                ...
                <div class="rounded-lg bg-white p-3">
                    ...
                    <p class="mt-1 text-lg font-semibold text-bakery-dark">
                        {{ trans("admin_production_plans.units.minutes", { count: summary.total_recipe_minutes }) }}
                    </p>
                </div>
                <div class="rounded-lg bg-white p-3">
                    <p class="text-xs uppercase tracking-[0.14em] text-bakery-brown/70">
                        {{ trans("admin_production_plans.summary.shortage_ingredients") }}
                    </p>
                    <p class="mt-1 text-lg font-semibold text-bakery-dark">
                        {{
                            trans("admin_production_plans.units.pieces", { count: summary.shortage_ingredients_count })
                        }}
                    </p>
                </div>
                <div class="rounded-lg bg-white p-3 sm:col-span-2 xl:col-span-4">
                    <p class="text-xs uppercase tracking-[0.14em] text-bakery-brown/70">
                        {{ trans("admin_production_plans.timeline.title") }}
                    </p>
